                                ...
                                {t.type === "college" ?
                                    <>
                                        <div className="task" onClick={() => openmodal(t.id)} style={{ cursor: "pointer" }}>
                                            {t.status === false ? <><div className="status">Done</div></> : <div className="status" style={{ background: "#ff3737" }} >Pending</div>}

                                            <p id="title" style={{ fontSize: "22px", fontWeight: "bold" }}> {t.title}</p>
                                            <div id="link"> <a href={t.links}>{t.links}</a> </div>
                                            <p id="note">Notes : {t.notes} </p>
                                            <p id="title">{new Date(t.created_on.seconds * 1000).toLocaleDateString("en-US")} to {new Date(t.completed_on.seconds * 1000).toLocaleDateString("en-US")} </p>
                    ...
                    <motion.div
                        ...
                        {Task.filter((t) => t.title.toLowerCase().includes(search.toLowerCase())).map((t) => {
                            return <>{t.type === "programming" ?
                                <>
                                    <div className="task" onClick={() => openmodal(t.id)} style={{ cursor: "pointer" }}>
                                        {t.status === false ? <><div className="status">Done</div></> : <div className="status" style={{ background: "linear-gradient(rgb(255, 0, 0), rgb(218 53 53))" }} >Pending</div>}
